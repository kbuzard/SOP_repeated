...
I really appreciate how closely you have read the manuscript and I am yet again grateful for your very constructive feedback along with that of the referees. I discuss how I have implemented each suggested change below, again writing a consolidated response. I have grouped your responses together where they are related and responded just once wherever possible, with my responses in italics. \\

There are minor grammatical changes throughout the text, but only Sections 2-6 contain any substantive additions or deletions. If you think it would expedite your review, I would be happy to provide you with a web link that details all of the changes in the .tex code.\\
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I have added a clause to this effect at the end of the paragraph following the un-numbered equation just above Equation (7) where $\tau^R(e_b)$ is introduced.}
\end{itemize}
...
I think the way you write and analyse the lobby's incentive constraint is still not very clear. I would explain things in the following sequence: (a) the relevant level of $e_b$ in constraint (8) is the best deviation effort, i.e. the optimal effort conditional on inducing a break of the agreement; (b) the best deviation effort is given by $\max \left\{e_{tw},\ov{e}(\bta)\right\}$; (c) in order for a non-trivial agreement to be possible, there must exist some $\tau^a< \tau^{tw}$ such that $e_{tw}<\ov{e}(\bta)$; (d) to avoid any confusion, I would write constraint (13) evaluated at $\max \left\{e_{tw},\ov{e}(\bta)\right\}$, rather than at $\ov{e}(\tau^a)$. Part of the reason I found the current exposition confusing is that your constraint (13) assumes $e_{tw}<\ov{e}(\tau^a)$, but this restriction is imposed and discussed only after writing constraint (13).

\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I have done exactly as you suggest just following constraint (8) and I have written constraint (11) evaluated at $\max \left\{e_{tw},\ov{e}(\bta)\right\}$ to address Referee 2's suggestions as well. I did not write equation (13) evaluated at this $\max \left\{e_{tw},\ov{e}(\bta)\right\}$ because it is meant to describe how the executives choose $\bta$, which necessitates raising $\bta$ high enough to ensure that $\ov{e}(\bta) > e_{tw}$. I instead imposed and discussed the restriction in the paragraph preceding the one that describes expression (13).}
\end{itemize}
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{Done throughout, including appendices.}
\end{itemize}
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I've changed the passage to specify that the lobby chooses exactly $e_{tw}$ and that the legislature best responds. In an earlier version with more complicated punishments, the more general specification helped to simplify the `punishments for the punishments' layer of analysis, but this is no longer necessary (equilibrium behavior was still the same, but it's quite true that I did not state this clearly). Thank you for pointing this out.}
\end{itemize}
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I have rearranged the key sentence to make clear that the political economy weights do not refer to the executives:}
	
	\begin{quote}
		The executives, as well as legislators with small political economy weights, prefer a strong-binding agreement.
	\end{quote}
	
	\textit{I have also stated clearly that this does not appear to match reality and have moved up from the appendix the possible justification I provided there.}
\end{itemize}
...
The discussion right after expression (14) is puzzling. If (14) is negative for all T, doesn't this mean that no tariff below $\tau^{tw}$ can be enforced? And if this is correct, why not make this point clearly - which might actually be an interesting theoretical point - rather than invoking some ad-hoc constraint on the minimum feasible punishment length, which I don't see any justification for?

\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{Expression (14) gives the derivative of the lobby's incentive constraint with respect to $T$. That is, it says how the slackness of the incentive constraint changes as we vary $T$. When (14) is negative, reducing $T$ makes the constraint more slack, or easier to satisfy. For there to be no tariff below $\tau^{tw}$ that is enforceable, we need that the lobby's constraint itself is negative. So this does not get around the---admittedly unsatisfying---need for some ad-hock constraint on the minimum feasible punishment length.}
\end{itemize}


\vskip.5in
\section{Simplify statement of Result 2}
\label{sec:sec8}
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I deleted the paragraph that was suggested and others like it in Section 5. I also cut and condensed throughout the text. There is more that could be cut, but I kept a bit to justify the assumptions and anything that---to the best of my memory and records---was added as a result of the editorial process at the JIE.}
	\item \textit{Addressing the other suggestions from this round added about one page of length. Through this tightening process, I reduced the total length by a little over four pages. }
\end{itemize}
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I first proofread the manuscript closely myself and then hired a professional to go over it again.}
\end{itemize}
...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I agree that this is awkward. I have tried to come up with some terminology and notation that means ``tries to disrupt the agreement but is not necessarily successful. I have not been at all successful. I apologize.}
\end{itemize}


\vskip.5in
\section{Consider adding a graph}
\textbf{Referee 2 Point 3}:\\
I suggest that the author consider adding a figure with $\tau$ on the y-axis and $e_b$ on the x-axis, and with an upward sloping line corresponding to $\tau^R(e)$. The author could then depict $\tau^a$ and $\tau^{tw}$ in ascending order on the y-axis, and similarly $e_a(\tau^a)$, $e_{tw}$ and $\ov{e}(\tau^a)$ in ascending order on the x-axis. Given $\tau^a$, we can find $e_a(\tau^a)$ off of the $\tau^R(e)$ curve, and similarly for $\tau^{tw}$ and $e_{tw}$. If the distance between $\ov{e}(\tau^a)$ and $e_{tw}$ is large in comparison to that between $e_{tw}$ and $e_a(\tau^a)$, then it can be seen that the cost to the lobby of going sufficiently above its ideal point, $e_{tw}$, could offset the future benefit of being eliciting $\tau^R(\ov{e}(\tau^a))$ and then $\tau^{tw}$ over the punishment phase. I may be mistaken, but I think this is the basic tradeoff that sits at the foundation of the analysis.

\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I found this suggestion appealing and tried to implement it but it quickly became complicated, in particular because the lobby's payoffs are in terms of $\pi(\tau^R)$, not $\tau^R$ directly. This added to the issue of aggregating across phases of the repeated game meant that the graph seemed to confuse more than clarify.}
\end{itemize}
