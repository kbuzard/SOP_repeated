...
I think the way you write and analyse the lobby's incentive constraint is still not very clear. I would explain things in the following sequence: (a) the relevant level of $e_b$ in constraint (8) is the best deviation effort, i.e. the optimal effort conditional on inducing a break of the agreement; (b) the best deviation effort is given by $\max \left\{e_{tw},\ov{e}(\tau^a)\right\}$; (c) in order for a non-trivial agreement to be possible, there must exist some $\tau^a< \tau^{tw}$ such that $e_{tw}<\ov{e}(\tau^a)$; (d) to avoid any confusion, I would write constraint (13) evaluated at $\max \left\{e_{tw},\ov{e}(\tau^a)\right\}$, rather than at $\ov{e}(\tau^a)$. Part of the reason I found the current exposition confusing is that your constraint (13) assumes $e_{tw}<\ov{e}(\tau^a)$, but this restriction is imposed and discussed only after writing constraint (13).

\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{I have done exactly as you suggest just following constraint (8) and I have written both constraint (11) and (13) evaluated at $\max \left\{e_{tw},\ov{e}(\tau^a)\right\}$ to address Referee 2's suggestions as well.}
\end{itemize}
