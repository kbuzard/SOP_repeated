...
\end{center}
\begin{equation}
  e_b \geq \pi(\tau^b(e_b)) - \left(\pi(\tau^a) - e_a \right) + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi(\tau^{tw}) -e_{tw} - \left(\pi(\tau^a) - e_a \right) \right]
  \label{ine:lob2}
\end{equation}

\begin{multline}
  \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \left[W_\text{ML}(\ga(e_a),\bta) - W_\text{ML}(\ga(e_{tw}),\btw) \right] \\
	- \left[ W_\text{ML}(\ga(\ov{e}),\tau^b(\ov{e}),\tau^{*a}) - W_\text{ML}(\ga(\ov{e}),\bta) \right] = 0
  \label{eq:leg2}
\end{multline}
\begin{equation}
  \ov{e}(\bta) - \left[ \pi(\tau^b(\ov{e})) - \left(\pi(\tau^a) - e_a \right) \right] - \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi(\tau^{tw}) -e_{tw} - \left(\pi(\tau^a) - e_a \right) \right] = 0
  \label{eq:lob2}
\end{equation}
...
\begin{itemize}
	\item Have extra term due to dependence on $e_b$. The total effect on $\Pi$ of $\ov{e}$ is the negative of the lobby's FOC. The lobby's FOC decreases to the right of $e_{tw}$ since $e_{tw}$ is the optimum. And $\ov{e}$ must be $\geq e_{tw}$ in equilibrium because this condition is required for the lobby's constraint to bind. So the effect of $\ov{e}$ is positive
\end{itemize}
