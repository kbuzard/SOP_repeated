...
becomes
				...
				\end{equation}
				\begin{itemize}
					\item The last bracketed term goes from positive in Expression~\ref{ine:lob2} to Expression~\ref{ine:lob3}
					\item The first bracketed term is largest at $\ov{e} = e_{tw}$
						\begin{itemize}
							\item When $\tau^p = \tau^{tw}$, we know $\ov{e} \geq e_{tw}$. When $\tau^p = 0$, $\ov{e}$ still $\geq e_{tw}$, but there is slack in the condition so that we could have a lower $\tau^a$ than the one that leads to $e_{tw}$.
						\end{itemize}
				\end{itemize}
