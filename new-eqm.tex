...
\begin{enumerate}
	...
	\item In any punishment period, lobby does not follow punishment: $e^D < e^N$
		\begin{itemize}
			\item Legislature chooses (??) BR to $e^D$, then restart at (1); if anything else, restart punishment from (2)
				\begin{itemize}
	...
	\item Checking that punishment is incentive compatible
		\begin{itemize}
			...
			\item Lobby:
				...
				Note that Expression~\ref{lobdev} will hold for all $T/i$.
					\begin{itemize}
						\item In last period of punishment, will hold with equality
...
\end{itemize}
	...
	From old construction, need to be rechecked:
		\begin{itemize}
			\item I've shown condition for lobby is constant through time except in last period, where they'll never pay (this was, obviously, for older model that doesn't enforce lobby paying in final period of punishment)
				\begin{itemize}
