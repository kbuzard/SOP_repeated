...
Section~\ref{sec:eqm} then explains the way in which the trade agreement negotiation process selects a particular class of equilibria and describes that class of equilibria. I describe the structure and properties of trade agreements in this environment in Section~\ref{sec:structure} and explore the forces shaping dispute resolution procedures in Section~\ref{sec:optimal}. Section~\ref{sec:asymmetric} explores an alternative punishment scheme. Section~\ref{sec:concl3} concludes.
...
The stage game is slightly more complex than in a standard repeated-game model of trade agreements in that each period of the repeated game has two phases. In the first phase, each lobby decides how much effort to exert to influence its respective legislature's tariff setting. In the second phase, the legislatures then set the applied tariff levels.\footnote{The executives move before the repeated game begins and do not have a role during play of the repeated game. Details are in Section~\ref{sec:zero}.}
...
Because this repeated game has a dynamic structure as described in Section~\ref{sec:stage}, it is important to carefully describe the informational set-up. To be clear, I will refer to one full play of the stage game as a ``period'' and I will use the term ``phase'' to describe within-stage-game divisions, i.e. the lobbying phase and the applied-tariff-setting phase. Although the assumption from the stage game that lobbying activity is not observable across international borders extends to the repeated game, that is, there is no learning across periods about lobbying effort, the tariff levels are perfectly observable across borders and across time, as well as within the stage game.
...
First, are public perfect equilibria (PPE) in a particular sense that is appropriate for the multi-phase stage-game. Given the game's structure and the assumption that lobbying effort is not observable across international borders, players in the same country can take advantage of more information than those who are in different countries. In equilibrium, I will assume that this extra information is only used within a period. That is, players condition on what they know from the current period and the publicly-observable tariff levels within a period. Across periods, however, players only condition on tariff levels. That is, the solution concept employed here is perfect public equilibrium (PPE) period to period. Whenever there is a possibility of multiple equilibria, I will focus on the one that maximizes the welfare of the executives. 

Second, I focus on a simple punishment scheme. For the results in Sections~\ref{sec:structure} and \ref{sec:optimal}, deviations from the trade agreement are punished by reverting to the stage game subgame-perfect Nash equilibrium for a specified number of periods before returning to cooperation. For convenience, I will refer to these punishments as `limited Nash reversion' punishments or `T-period Nash reversion' punishments. They represent a trade war that is limited in duration and therefore more realistic than infinite Nash reversion.
...
\label{sec:zero}
Given the punishment length $T$, the executives determine the specific equilibrium by choosing the trade agreement tariffs, which I assume take the form of tariff caps, to maximize joint social welfare. The executives face the constraint that the trade agreement tariff caps they choose must be consistent with equilibrium play by the legislatures and lobbies. 

The executives have no other opportunity to affect the outcome of the trading relationship. Thus the executives maximize joint welfare subject to the incentive constraints of the repeated-game players, and one can view their choice of the trade agreement tariffs as setting a key parameter for the repeated game.
...
\begin{equation}
  ...
  \label{ine:lob}
\end{equation}
Section~\ref{sec:structure} explores the implications of these incentive constraints for the structure of the equilibrium trade agreement.

\subsection{Equilibrium with Limited Nash Reversion Punishments}
\label{sec:nashrev}
Unlike with infinite Nash reversion punishments, $T$-period Nash reversion punishments are not necessarily public perfect since the players may want to influence the future path of play during punishment periods. Public perfection can be ensured by specifying that all players ignore any deviations from the punishment by any other player. Note again that, although the stage game has two phases and the players within a country may use non-public information within a stage game, players only employ public information across periods.
...
Having fully described the strategies accompanying this punishment scheme, it must be shown that they constitute a public perfect equilibrium, i.e. for each date and history, the strategies constitute a Nash equilibrium.
...
First examine the incentives for the legislature to adhere to the punishment. The legislature is supposed to play its one-shot best response to the lobby's effort choice. This implies that there is no deviation that creates a stage-game improvement for the legislature. Because the other players will ignore any deviation, the continuation payoff also cannot be improved upon because it does not depend on the actions that are chosen at time $t$. Thus there is no profitable deviation for the legislature in any period of the punishment.

The lobby's incentives for adhering to the punishment are similar. Its stage-game net profits are maximized at the prescribed effort level $e_{tw}$ given the legislature's myopic best response. There is no deviation that will provide higher stage-game payoffs given the stage-game subgame-perfect Nash equilibrium strategies. And since all players' strategies specify that deviations are ignored, play during a punishment sequence is not conditioned on what happens from period to period. That is, there is no profitable deviation for the lobby from the prescribed strategy during a punishment.
...
\begin{result}
  In the case of political certainty, the equilibrium trade agreement induces lobbying effort aimed at achieving the tariff cap only and is never subject to dispute. The executives choose the minimum tariff level that induces the lobby to choose $e=e_a$.
  \label{res:eqm}
\end{result}
...
The above-explored symmetric, limited Nash-reversion punishments are not the only possible punishments. This section explores an asymmetric punishment scheme in which welfare is reduced for both the legislature and the lobby in the defecting country. Although this type of punishment is hard to find in practice, I show here that in some cases it allows for increased cooperation in the form of lower trade agreement tariffs.

In this scheme, instead of $T$ periods of Nash reversion, the legislature in the defecting country is required to apply a zero tariff for $T$ periods, with an accompanying effort level of zero by the lobby. To keep the analysis simple, the non-defecting country's strategies are kept the same as in the limited Nash reversion case.

In order to compare the trade agreement tariffs under the two punishment schemes, I assume that both are interior in the sense that they are strictly less than $\tau^{tw}$ themselves. Compared to the legislature's constraint under limited Nash reversion (Expression~\ref{eq:leg2}), the only change induced by the alternative punishment scheme is a reduction in the punishment tariff from $\tau^{tw}$ to zero. This term in the constraint changes from $W_\text{ML}(\ga(\ov{e}),\tau^{tw},\tau^{*tw})$ to $W_\text{ML}(\ga(\ov{e}),0,\tau^{*tw})$.
...
Incentive compatibility depends on the full equilibrium strategies that are specified. It has just been shown that replacing the $T$-period Nash reversion punishments with a $T$-period punishment of zero tariffs and zero effort allows for a lower trade agreement tariff. Because the structure of these punishments for deviating from the trade agreement is different from that analyzed earlier in the paper, it's necessary to find new punishments for deviating from the alternative punishments.

The separability of the economy permits a focus on the home economy in the following way. The best response functions of the foreign actors are independent of the level of the tariff chosen by the home legislature, so the actors in the foreign county---here taken to be ones who are not deviating from the trade agreement---can be instructed to play the stage game SPNE strategies. If they are also instructed to ignore any deviations by the home actors, and any deviations by them are ignored by the home actors, the reasoning from Section~\ref{sec:nashrev} goes through to establish incentive compatibility.

I will show that the following punishment scheme allows for incentive compatibility of the trade agreement for any set of parameters, i.e. for any economy. Begin by examining the lobby's incentives for deviating from the punishment. Given that the legislature's action in a punishment period is to choose $\tau^p =0$ regardless of the lobby's choice and that the lobby is already exerting the lowest-cost effort level, the lobby cannot improve its payoff by deviating in the current period. The lobby's incentive constraint will hold if we specify that in the remaining periods of any punishment after a deviation by the lobby, the players play the original punishment actions. In effect, any deviation by the lobby is ignored.
...
I complete the discussion by checking that these punishments for deviating from the main punishments are incentive compatible. Start by examining any subgame in which the legislature has just deviated from the main punishment. The lobby has no incentive to deviate from enforcing the legislature's punishment for deviating from the punishment $\left(0,\tau^P\right)$ as, again, the lobby is already exerting the minimum effort and any deviation it makes will be ignored. Thus ignoring any deviation by the lobby suffices to satisfy the lobby's incentives in this situation as well. In order to make this punishment incentive compatible for the legislature, we specify that any deviation from it triggers a re-starting of the punishment.

All that is left is to check that the legislature is happy to ignore any deviation by the lobby from the primary punishment. Recall that the lobby's best deviation, if the legislature were to respond, would be to $e_{tw}$, and consider a punishment like the one in Expression~\ref{eq:legpunasym} except that it is of length $S_2$:
\begin{multline}
