...
\begin{equation}
  \ov{e}(\bta) - \left[ \pi_X(\tau^b(\ov{e}(\bta))) - \pi_X(\tau^a) + e_a(\tau^a)\right] - \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi_X(\tau^{tw}) -e_{tw} - \pi_X(\tau^a) + e_a(\tau^a) \right] = 0
  \label{eq:lob2}
\end{equation}
...
\noindent The intuition is as follows: $\ov{e}(\bta)$ must be at least as large as $e_{tw}$ in equilibrium because the lobby's net profits are maximized at $e_{tw}$. If $\ov{e}(\bta) < e_{tw}$, the lobby's constraint will not be satisfied since the lobby will exert effort level $e_{tw}$. Since the trade agreement tariff is weakly less than the trade war tariff, the median legislator's most preferred tariff at $\ov{e}(\bta)$---that is, $\tau^b\left(\ov{e}(\bta)\right)$---must be higher than the trade agreement tariff. Raising $\tau^a$ brings the trade agreement tariff closer to the legislature's ideal point, requiring the lobby to pay more to make the legislature willing to break the agreement.
