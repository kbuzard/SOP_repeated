...
\begin{multline}
  \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \left[W_\text{ML}(\ga(\ov{e}),\bta) - W_\text{ML}(\ga(\ov{e}),\btw) \right] \\
	- \left[ W_\text{ML}(\ga(\ov{e}),\tau^b(e_b),\tau^{*a}) - W_\text{ML}(\ga(\ov{e}),\bta) \right] = 0
  \label{eq:leg2}
\end{multline}
...
\begin{equation}
  \ov{e}(\bta) - \left[ \pi(\tau^b(e_b)) - \pi(\tau^a) \right] - \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi(\tau^{tw}) -e_{tw} - \pi(\tau^a) \right] = 0
  \label{eq:lob2}
\end{equation}
...
\begin{equation}
  ...
  \left[ \pi(\tau^b(e_b)) - \pi(\tau^a) \right] + T \left[ \pi(\tau^{tw}) - \pi(\tau^a) \right]\right\}} - \left[ \pi(\tau^{tw}) - e_{tw} -\pi(\tau^a) \right]
 	\label{ine:Tdelta1}
\end{equation}
...
\begin{equation}
	...
	\textstyle \frac{\left[1+ \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}  \right]\frac{\partial}{\partial \tau^a}W_\text{ML}(\ga(\ov{e}),\bta)} {\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^a) - \pi(\tau^{tw}) \right] - \frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^b(e_b)) - \pi(\tau^{a}) \right]}
	\label{eq:coret}
\end{equation}

\noindent In order for the median legislator to be made indifferent (that is, for Equation~\ref{eq:leg2} to hold), $\ga(\ov{e})$ must be such that the cheater payoff $W_\text{ML}(\ga(\ov{e}),\tau^b(e_b),\tau^{*a})$ is higher than the payoff at $\tau^{tw}$ \textit{and} such that the same is true of $\tau^a$ and $\tau^{tw}$. Therefore $\ov{e}$ will be set so that the median legislator's ideal point is to the right of $\tau^a$ (and to the left of $\tau^{tw}$), implying that the numerator is positive.
...
\begin{multline}
  \mathrm{e}^{-r\Delta}\left(1-\mathrm{e}^{-r\Delta T}\right)\left[W_\text{ML}(\ga(e),\bta) - W_\text{ML}(\ga(e),\btw) \right] - \\ \left(1-\mathrm{e}^{-r\Delta}\right) \left[W_\text{ML}(\ga(e),\tau^b(e_b),\tau^{*a}) - W_\text{ML}(\ga(e),\bta) \right]   \geq 0
\end{multline}
\vskip-.4in
\begin{multline}
...
\[
  \left(1-\mathrm{e}^{-r\Delta}\right) \left[\pi(\tau^b(e_b)) - \pi(\tau^a_\text{NL}) \right] + \mathrm{e}^{-r\Delta}\left(1-\mathrm{e}^{-r\Delta T}\right) \left[\pi(\tau^{tw}) - e_{tw} - \pi(\tau^a_\text{NL}) \right] 
\]
...
\begin{equation}
	...
	\frac{ \frac{1 - \left(T+1\right)\de_\text{ML}^T + T \de_\text{ML}^{T+1}}{\left(1-\de_\text{ML} \right)^2} \left[  W_\text{ML}(\ga(\ov{e}),\bta) - W_\text{ML}(\ga(\ov{e}),\btw) \right]}{\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^a) - \pi(\tau^{tw}) \right] - \frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^b(e_b)) - \pi(\tau^{a}) \right]}
 	\label{eq:e_de}
\end{equation}
