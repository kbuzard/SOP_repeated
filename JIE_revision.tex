...
\begin{enumerate}
	...
	\item Conditions in (1) held in period $t-2$, but there was a violation in period $t-1$
		...
		\end{itemize}
	\item Static Nash punishment was initiated $j<T$ periods ago, and punishment has been followed since then, i.e. $\tau^D \geq \tau^N$ and $e^D \geq e^N$ $\forall t \in \left[t-j,j \right]$ 
		\begin{itemize}
			\item Punish this period and $T-j-1$ more periods before switching back to (1)
		\end{itemize}
