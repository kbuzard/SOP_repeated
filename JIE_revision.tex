...
In Section~\ref{sec:stage}, I claim that the government welfare function presented in this paper can be interpreted as a special case of the general welfare function proposed in \citet{dgh97}. Here I show that, under the non-unitary government assumption, the \citet{dgh97} model specialized to this environment produces results that are qualitatively the same as those in the main body of the paper.

Here, the lobby offers a contribution schedule of pairs of effort levels and tariff. Following \citet{dgh97}, I assume these pairs reflect the fact that the lobby has all the bargaining power in its relationship with the government.

Denote the government's welfare function as $W_G(\tau,\tau^*,e(\tau)) = W(\tau,\tau^*) + g(e(\tau))$ where $W = CS_X + PS_X + CS_Y + PS_Y +TR$ is social welfare. The structure of the trade-agreement-setting phase does not change (Section~\ref{sec:zero}). In the trade war, the government will be presented with a contribution schedule of lobbying effort, tariff pairs that can be denoted as a function $e(\tau)$. The government then maximizes $W_G(\tau,\tau^*,e(\tau))$ unilaterally.

Given Propositions 1 and 3 of \citet{dgh97} and the assumption that the lobby has all the bargaining power, the lobby calculates $\left(\tau,e\right)$ schedule from $W(\tau) +g(e) = W(\tau^{opt}) +g(0)$ where $\tau^{opt}$ is the government's optimal unilateral tariff in the absence of lobbying effort. If we assume $g(0) = 0$, $e = g^{-1}\left[W(\tau^{opt}) - W(\tau)\right]$.
								
The break phase is also altered. Here, the legislature's repeated-game incentive constraint is altered qualitatively and therefore the lobby's incentive constraint changes quantitatively (although not qualitatively). Equation~\ref{eq:leg2}, which defines how much the lobby must pay to break the trade agreement (i.e. $\ov{e}$), must therefore be re-written.

When lobby has all the bargaining power, the $(\ov{e},\tau^b)$ schedule makes the legislature indifferent between breaking the trade agreement or abiding by it, maximizing the lobby's future income stream by paying the legislature no more than necessary to break the break agreement. Note that in contrast to the trade war environment, if the legislature doesn't break the trade agreement, the institutional environment is such that the outside option is $\left(\tau^a,e_a\right)$. The legislature's incentive constraint is then
	\begin{multline}
...
The main construction in Section~\ref{sec:structure} remains qualitatively the same, as Lemmas~\ref{lem:et} and \ref{lem:ets} continue to hold. 
					
Lemma 1:
	\begin{equation*}
		\frac{\mathrm{d} \ov{e}}{\mathrm{d} \tau^a} = -\frac{\frac{\partial \Omega}{\partial \tau^a}}{\frac{\partial \Omega}{\partial \ov{e}}} =
	\textstyle \frac{\left[1+ \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}  \right]\frac{\partial}{\partial \tau^a}\left[W(\bta) + g(e_a(\tau^a))\right]}{\left[1+\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\right] \frac{\partial g(e)}{\partial \ov{e}} }
  \end{equation*}
...
The denominators have the same sign, but are different from the main model in that the lobby's profit differentials do not appear here because the $g(\cdot)$ function is additively separable from profits, whereas $\ga(\cdot)$ is not. Their numerators are not changed much: $W(\bta) + g(e_a(\tau^a))$ is the equivalent of $W_{ML}(\ga(e_a),\bta)$ in this alternative model. The substantive change is that the expression is evaluated at $e_a$ instead of $\ov{e}$ because indifference must be created here by contrast to the outside option. 
...
\newpage
As for the results on optimal dispute settlement, with the government incentive constraint altered to Expression~\ref{eq:newleg}, Expression~\ref{ine:T} for the change in the constraint when $T$ changes is 
	\begin{multline}
		\left(1 - \frac{\mathrm{d} \pi}{\mathrm{d} \ov{e}} \right) \frac{ -\frac{\de_\text{ML}^{T+1}\ln\de_\text{ML}}{1-\de_\text{ML}}\left[  W(\bta) +g(e_a(\tau^a)) - W(\btw) - g(\ov{e}) \right]}{\left[ 1 + \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \right] \frac{\partial g(e)}{\partial e} } \\
	+  \frac{\de_\text{L}^{T+1} \ln \de_\text{L}}{1-\de_\text{L}} \left[ \pi(\tau^{tw}) - e_{tw} -\pi(\tau^a) + e_a(\tau^a) \right]
	\end{multline}
The only change is to the expression for $\frac{\partial \ov{e}}{\partial T} \rightarrow 0$. The denominator changes as in Lemmas~\ref{lem:et} and \ref{lem:ets} and converges to $\frac{\de}{1-\de}$ as $T \rightarrow \infty$ just as in the main formulation. Likewise, the numerator converges to $0$.

\newpage
We do need the condition that $W(\bta) +g(e_a(\tau^a)) > W(\btw) + g(\ov{e})$, that is, that the trade war is actually a punishment for the government. But the lobby must choose the $\tau^b,\ov{e}$ pairs via Equation~\ref{eq:newleg} to make $W(\tau^b,\tau^{*a}) +g(\ov{e}) > W(\bta) + g(e_a(\tau^a))$ since $\tau^b$ must be greater than $\tau^a$ in order for the trade agreement to be violated. This implies the previous inequality.
