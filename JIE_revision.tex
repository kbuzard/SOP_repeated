...
In Appendix~\ref{sec:dgh}, I demonstrate that an appropriately-stylized version of the \citet{dgh97} model produces results that that are, in fact, qualitatively similar to those of the model presented here. It is also easy to show that the results of the model are unchanged if the lobby's effort is subtracted from the executive's and/or the median legislator's welfare function. I therefore do not include it in order to maintain consistency with the literature (e.g. \citet{gh94} and \citet{mrc2007} where utility is transferable between the government and lobby and \citet{dgh97} and \citet{lt}).
...
\begin{multline}
  ...
  \label{ine:leg}
\end{multline}
Built into this condition is the legislature's applied tariff-setting behavior when the lobby's effort level is below the cutoff value $\ov{e}(\bta)$ that leads the legislature to break the trade agreement. Label the effort level at which the legislature chooses a particular $\tau^a$ as its optimal unilateral tariff as $e_a(\tau^a)$. The determination of $\ov{e}(\bta)$ is described in the next section. For any effort level weakly between $e_a(\tau^a)$ and $\ov{e}(\bta)$, the legislature chooses $\tau^a$ as the applied tariff. If the effort level is below $e_a(\tau^a)$, the legislature chooses the corresponding applied tariff, which is necessarily less than $\tau^a$. Because the lobby's net profits are highest at $\tau^{tw}$, when the lobby does not choose $\ov{e}(\bta)$, it will necessarily choose $e_a(\tau^a)$ and the applied tariff will be $\tau^a$.
...
\begin{multline}
  \pi_X(\tau^a) - e_a(\tau^a) + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi_X(\tau^a) - e_a(\tau^a) \right] \geq \\ \pi_X(\tau^R(e)) - e + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi_X(\tau^{tw}) - e_{tw} \right] .
  \label{ine:lob}
\end{multline}
...
\end{center}
\begin{equation}
  e \geq \pi_X(\tau^R(e)) - \pi_X(\tau^a) + e_a(\tau^a) + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi_X(\tau^{tw}) -e_{tw} - \pi_X(\tau^a) + e_a(\tau^a)\right]
  \label{ine:lob2}
\end{equation}
...
At time $t=1$, in any period following a period when the trade agreement has been adhered to, or after the successful completion of a punishment, the lobby chooses $e =e_a(\tau^a)$ and the legislature chooses $\tau^a$, that is, to abide by the agreement by implementing the tariff cap.
...
\begin{equation}
  \ov{e}(\bta) - \left[ \pi_X(\tau^b(\ov{e})) - \pi_X(\tau^a) + e_a(\tau^a)\right] - \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi_X(\tau^{tw}) -e_{tw} - \pi_X(\tau^a) + e_a(\tau^a) \right] = 0
  \label{eq:lob2}
\end{equation}
...
\[
  e \geq \pi_X(\tau^b(e)) - \pi_X(\tau^a) + e_a(\tau^a) + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi_X(\tau^{tw}) - e_{tw} -\pi_X(\tau^a) + e_a(\tau^a) \right]
\]
...
\begin{multline}
  ...
  \left[ \pi_X(\tau^b(\ov{e})) - \pi_X(\tau^a) \right] + T \left[ \pi_X(\tau^{tw}) - \pi_X(\tau^a) \right]\right\}} \\ - \left[ \pi_X(\tau^{tw}) - e_{tw} -\pi_X(\tau^a) + e_a(\tau^a) \right]
 	\label{ine:Tdelta1}
\end{multline}
...
\begin{equation}
	...
	\frac{ \frac{1 - \left(T+1\right)\de_\text{L}^T + T \de_\text{L}^{T+1}}{\left(1-\de_\text{L} \right)^2} \left[\pi_X(\tau^{tw}) -e_{tw} - \pi_X(\tau^a) + e_a(\tau^a)\right]}{\left(1 + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}}\right)\left[\frac{\partial \pi_X(\tau^a)}{\partial \tau^a} - \frac{\partial e_a(\tau^a)}{\partial \tau^a}\right]}
\end{equation}
...
\begin{equation*}
	...
	\textstyle \frac{\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \frac{\partial}{\partial \tau^{*a}}W_\text{ML}(\ga(e_a(\tau^a)),\bta)}{\frac{\partial \ga}{\partial \ov{e}}\left[ \pi_X(\tau^b(\ov{e})) - \pi_X(\tau^{a}) \right]}
\end{equation*}
...
\begin{multline}
 	\left(1 - \frac{\mathrm{d} \pi_X}{\mathrm{d} \ov{e}} \right) \frac{ -\frac{\de_\text{ML}^{T+1}\ln\de_\text{ML}}{1-\de_\text{ML}}\left[  W_\text{ML}(\ga(e_a(\tau^a)),\bta) - W_\text{ML}(\ga(e_{tw}),\btw) \right]}{\frac{\partial \ga}{\partial e} \left[ \pi_X(\tau^b(\ov{e})) - \pi_X(\tau^a) \right]} \\
	+  \frac{\de_\text{L}^{T+1} \ln \de_\text{L}}{1-\de_\text{L}} \left[ \pi_X(\tau^{tw}) - e_{tw} -\pi_X(\tau^a) + e_a(\tau^a) \right]
	\label{eq:uniopt}
\end{multline}
...
As for the results on optimal dispute settlement, with the government incentive constraint altered to Expression~\ref{eq:newleg}, Expression~\ref{eq:uniopt} for the change in the constraint when $T$ changes is 
	\begin{multline}
		\left(1 - \frac{\mathrm{d} \pi}{\mathrm{d} \ov{e}} \right) \frac{ -\frac{\de_\text{ML}^{T+1}\ln\de_\text{ML}}{1-\de_\text{ML}}\left[  W(\bta) +g(e_a(\tau^a)) - W(\btw) - g(e_{tw}) \right]}{ \frac{\partial g(e)}{\partial e} } \\
