...
The lobby chooses its effort $e_p$ given the above tariff-setting behavior by maximizing its profits net of effort: $\pi\left(\tau^{tw}\left(\ga\left(e_{p}\right)\right)\right) - e_{p}$. (Note this is Equation~\ref{eq:lv3} simplified by the resolution of uncertainty over the legislature's decision on the trade agreement). This implies a first order condition of
\begin{equation}
	\frac{\partial \pi(\tau^{tw}(\ga(e_p)))}{\partial e_{p}} = 1
  \label{eq:lobtw}
\end{equation}
...
\label{sec:nashrev}
I now turn to a full description of the strategies and incentives for the equilibrium that has been analyzed in the preceding sections. Again, this is a symmetric equilibrium, so I describe strategies for the home country only; similar conditions hold for the foreign country.

In the first period of the game, in any period following a period when the trade agreement has been adhered to, after the successful completing of a punishment, or after a violation of the punishment by the lobby, the lobby chooses $e_b =0$, the legislature chooses to abide by the agreement, the lobby chooses $e_a$ and the legislature chooses $\tau^a$ as the applied tariff.
...
In any period in which the punishment is supposed to be followed, if the legislature chooses a tariff that is below its unilateral best response to $e_p$, 
...
\begin{enumerate}
	...
	\item In any punishment period, lobby does not follow punishment: $e^D < e^N$
		\begin{itemize}
			\item Legislature chooses its BR to $e^D$ (i.e. $\tau^D(e^D))$), then restart at (1); if legislature does anything else, restart punishment from (2). BR + eqm, so okay
				\begin{itemize}
