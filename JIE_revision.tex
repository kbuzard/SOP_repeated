...
Compared to the legislature's constraint under limited Nash reversion (Expression~\ref{eq:leg2}), the only change induced by the alternative punishment scheme is a reduction in the punishment tariff from $\tau^{tw}$ to zero. 

This results in an upward shift of the $\left(\bta,\ov{e}\left(\bta\right) \right)$ function. Intuitively, the punishment becomes harsher for the legislature, so the lobby has to exert more effort to achieve a break at any given level of $\bta$.

With this result in hand, we turn to the lobby's constraint. Here the changes are more complex. First, the future payoff difference goes from positive to negative. The lobby no longer benefits from provoking a dispute. This serves to loosen the constraint. Second, net profits decrease under the alternative punishment scheme because the $\ov{e}$ required to break the agreement is higher. Taken together, these facts imply that changing the punishment scheme creates slack in the lobby's constraint. This slack can be exploited to reduce $\bta$. The analysis just completed, along with the following treatment of incentive compatibility, leads to the following result:
...
\label{res:asym}
\end{result}
Thus this punishment scheme that is disliked by the lobby as well as the legislature can support lower trade agreement tariffs when it can be sustained.\footnote{Note that a different set of `punishments for the punishments' is required to ensure incentive compatibility. It is easy to show that incentive compatibility holds in general. However, the punishment length required is in general different than under limited Nash-reversion punishments.} Switching to this alternative punishment scheme amounts to selecting a different equilibrium; it's not clear how possible this switch might be given the political power of the lobbies whose welfare would be reduced under both the lower equilibrium trade agreement tariffs and the punishment tariffs if the environment were such that it was possible to get off the equilibrium path.
