...
%$\ov{e}(\bta)$ must be at least as large as $e_{tw}$ in equilibrium because the lobby's net profits are maximized at $e_{tw}$. 
...
\begin{equation}
 	\frac{\mathrm{d} \ov{e}}{\mathrm{d} \tau^{*a}} = -\frac{\frac{\partial \Omega}{\partial \tau^{*a}}}{\frac{\partial \Omega}{\partial \ov{e}}} = -
	\textstyle \frac{\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \frac{\partial}{\partial \tau^{*a}}W_\text{ML}(\ga(\ov{e}),\bta)} {\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\frac{\partial \ga(\ov{e})}{\partial e}\left[ \pi_X(\tau^a) - \pi_X(\tau^{tw}) \right] - \frac{\partial \ga(\ov{e})}{\partial e}\left[ \pi_X(\tau^b(\ov{e}(\bta))) - \pi_X(\tau^{a}) \right]}
	\label{eq:corets}
\end{equation}
...
\]
The lobby's FOC decreases to the right of $e_{tw}$ since $e_{tw}$ is the optimum $\left( \frac{\mathrm{d} \pi_X}{\mathrm{d} \ov{e}} = 1 \text{ at } e = e_{tw} \right)$. Since we must have $\ov{e}(\bta) \geq e_{tw}$ in equilibrium in order for the lobby's constraint to bind, the effect of $\ov{e}$ on $\Pi$ is positive. In addition, $\frac{\mathrm{d} \ov{e}}{\mathrm{d} \de_\text{ML}}$ is positive by Corollary~\ref{cor:edm}, so the numerator is positive. 
...
This means that nothing changes in the trade war. Likewise nothing changes in the legislature's repeated-game incentive constraint, and so Lemmas~\ref{lem:et} and \ref{lem:ets} are undisturbed by the change to strong bindings.

It is the lobby's incentive constraint that changes since the lobby no longer need pay $e_a$ for the trade agreement tariff. This makes the lobby's incentive constraint easier to satisfy for strong bindings at a given $\bta$. This can be seen in the next inequality, where the right-hand side of the incentive constraint becomes smaller:
\begin{multline*}
