...
\begin{equation}
  \max_{\bta} \frac{\bm{W_\text{E}}(\bta)}{1-\de_\text{E}} \hskip.2in \text{subject to } (\ref{ine:leg2}) \text{ and } (\ref{ine:lob2})
  \label{prob:max}
\end{equation}
\begin{multline}
  \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \left[W_\text{ML}(\ga(e_b),\bta) - W_{\text{ML}}(\ga(e_b),\btw) \right] \geq \\
	W_{\text{ML}}(\ga(e_b),\tau^b,\tau^{*a}) - W_{\text{ML}}(\ga(e_b),\bta)
  \label{ine:leg2}
\end{multline}
\begin{equation}
  e_b \geq \pi(\tau^b) - \pi(\tau^a) + \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi(\tau^{tw}) -e_{tw} - \pi(\tau^a) \right]
  \label{ine:lob2}
\end{equation}
...
In state $C$, the lobby has a two-stage problem. First, for the given $\bta$, $\bm{\de}$ and $T$, it calculates the minimum $e_b$ required to induce the legislature to break the trade agreement. Call this minimum effort level $\ov{e}(\bta)$. This calculation of precise indifference is possible because it is assumed here that the political process is certain---that is, all actors know precisely how lobbying effort affects the identity of the median legislator through $\ga(e_b)$.

Given the $e_b$ required to break the agreement, the lobby will then compare its current and future payoffs from inducing a dispute net of lobbying effort $\left(\text{that is, }\pi(\tau^b) + \de_\text{L} V^D_L - \ov{e}\right)$ to the profit stream from the trade agreement with no lobbying effort $\left(\pi(\tau^a) + \de_\text{L} V^C_L\right)$. With the appropriate substitutions and rearrangements, this is just Condition (\ref{ine:lob2}) evaluated at $\ov{e}$. If the former is larger, it induces the cheapest possible break; if the latter is larger, the lobby chooses to be inactive and the agreement remains in force.

The executives maximize social welfare by choosing the lowest tariffs such that the trade agreement they negotiate remains in force. Thus they must raise tariffs to the point that makes the lobby indifferent between exerting effort $\ov{e}(\bta)$ and disengaging completely,\footnote{Here I assume that the lobby does not exert effort when indifferent; if one were to assume the opposite, tariffs would have to be raised by an additional $\varepsilon$.} provided that this also satisfies the legislative constraint. By construction, the legislative constraint will always be slack because the $\ov{e}(\bta)$ schedule is calculated to make the median legislator indifferent between cooperating and initiating a dispute but then in equilibrium, $\bta$ is chosen so that the lobby disengages. When the lobby is disengaged $\left(e_b=0\right)$, the median legislator cannot prefer to break the agreement since her preferred tariff is lower when $e_b=0$ than when $e_b=\ov{e}(\bta)$.
...
\begin{multline}
  \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \left[W_\text{ML}(\ga(\ov{e}),\bta) - W_\text{ML}(\ga(\ov{e}),\btw) \right] \\
	- \left[ W_\text{ML}(\ga(\ov{e}),\tau^b,\tau^{*a}) - W_\text{ML}(\ga(\ov{e}),\bta) \right] = 0
  \label{eq:leg2}
\end{multline}
...
\begin{equation}
  \ov{e}(\bta) - \left[ \pi(\tau^b) - \pi(\tau^a) \right] - \frac{\de_\text{L} - \de_\text{L}^{T+1}}{1-\de_\text{L}} \left[\pi(\tau^{tw}) -e_{tw} - \pi(\tau^a) \right] = 0
  \label{eq:lob2}
\end{equation}
...
A central question is how the two equations are linked, that is, what is the nature of the relationship between $\tau^a$ and $\ov{e}$, and it is straightforward. 
\begin{corollary}
...
\noindent Because $\ov{e}$ makes the median legislator indifferent between the trade agreement tariff and the higher ``cheater's'' tariff balanced by a future punishment, we know that the median legislator's most preferred tariff when $e_b=\ov{e}$ must be higher than the trade agreement tariff. Therefore raising $\tau^a$ brings the trade agreement tariff closer to the legislature's ideal point, and thus the lobby must pay more to make the legislature willing to break the agreement.

Corollary~\ref{cor:et} helps to answer the comparative static question of how changes in the patience level of the lobby effect the equilibrium trade agreement tariffs.
...
\begin{equation}
  ...
  \left[ \pi(\tau^b) - \pi(\tau^a) \right] + T \left[ \pi(\tau^{tw}) - \pi(\tau^a) \right]\right\}} - \left[ \pi(\tau^{tw}) - e_{tw} -\pi(\tau^a) \right]
 	\label{ine:Tdelta1}
\end{equation}
...
\bibliography{C:/Users/Kristy/Dropbox/Research/xBibs/tradeagreements}
...
{\hypertarget{Lem3}{\begin{lemma}}
  A solution to the executives' problem (\ref{prob:max}) exists for all $\de$ and all $T$.
  \label{lem:exist}	 
\end{lemma}
Proof: The executives' problem is to minimize $\bta$ such that both the legislature's and the lobby's constraints are satisfied. If the solution to the problem in the absence of lobbies (i.e. with only the legislature's constraint, and that evaluated at $e_b=0$) cannot be satisfied for any $\bta < \btw$ (that is, $\bta_\text{NL} = \btw$), then the solution to (\ref{prob:max}) will also be $\btw$.
...
\begin{multline}
  \mathrm{e}^{-r\Delta}\left(1-\mathrm{e}^{-r\Delta T}\right)\left[W_\text{ML}(\ga(e),\bta) - W_\text{ML}(\ga(e),\btw) \right] - \\ \left(1-\mathrm{e}^{-r\Delta}\right) \left[W_\text{ML}(\ga(e),\tau^b,\tau^{*a}) - W_\text{ML}(\ga(e),\bta) \right]   \geq 0
\end{multline}
\vskip-.4in
\begin{multline}
  \left(1-\mathrm{e}^{-r\Delta}\right) e_b - \left(1-\mathrm{e}^{-r\Delta}\right) \left[\pi(\tau^b) - \pi(\tau^a) \right] - \\
	\mathrm{e}^{-r\Delta}\left(1-\mathrm{e}^{-r\Delta T}\right) \left[\pi(\tau^{tw}) - e_{tw} - \pi(\tau^a) \right] \geq 0
  \label{ine:lobnorm}
\end{multline}
The proof is via the Intermediate Value Theorem. I take as the leftmost boundary $\bta_\text{NL}$ (the trade agreement chosen by the executives when there is no lobby) because this is the lowest possible tariff the executives can achieve before the additional constraint implied by the presence of lobbies is added. By construction, $\ov{e}(\bta_\text{NL})=0$. The gain to the lobby of a break in the trade agreement here is 
\[
  \left(1-\mathrm{e}^{-r\Delta}\right) \left[\pi(\tau^b) - \pi(\tau^a_\text{NL}) \right] + \mathrm{e}^{-r\Delta}\left(1-\mathrm{e}^{-r\Delta T}\right) \left[\pi(\tau^{tw}) - e_{tw} - \pi(\tau^a_\text{NL}) \right] 
\]
If this gain is non-positive, the lobby has no incentive to exert effort and so $\bta_\text{NL}$ is the solution to the executives' problem.\footnote{Note, in particular, that this is the case if both $\Delta$ and $\Delta T \rightarrow 0$. This is also possible for some very large $\tau^a_\text{NL}$ if $\pi(\tau^{tw}) - e_{tw} - \pi(\tau^a_\text{NL})< 0$, a case that is beyond the scope of this paper. See the proof of Corollary~\ref{cor:tdl} and the Conclusion for a discussion.} If the lobby's gain is strictly positive, the left-hand size of Expression~\ref{ine:lobnorm} is negative. The executives will have to raise the trade-agreement tariff to prevent the agreement from being broken in this case because the lobby's constraint is not satisfied.
...
\vskip.4in
\noindent \textbf{\hypertarget{Cor_et}{Proof of Corollary~\ref{cor:et}}}: \\
...
\begin{equation}
	...
	\textstyle \frac{\left[1+ \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}  \right]\frac{\partial}{\partial \tau^a}W_\text{ML}(\ga(\ov{e}),\bta)} {\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^a) - \pi(\tau^{tw}) \right] - \frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^b) - \pi(\tau^{a}) \right]}
	\label{eq:coret}
\end{equation}

\noindent In order for the median legislator to be made indifferent (that is, for Equation~\ref{eq:leg2} to hold), $\ga(\ov{e})$ must be such that the cheater payoff $W_\text{ML}(\ga(\ov{e}),\tau^b,\tau^{*a})$ is higher than the payoff at $\tau^{tw}$ \textit{and} such that the same is true of $\tau^a$ and $\tau^{tw}$. Therefore $\ov{e}$ will be set so that the median legislator's ideal point is to the right of $\tau^a$ (and to the left of $\tau^{tw}$), implying that the numerator is positive.
...
\begin{equation}
	...
	\frac{ \frac{1 - \left(T+1\right)\de_\text{ML}^T + T \de_\text{ML}^{T+1}}{\left(1-\de_\text{ML} \right)^2} \left[  W_\text{ML}(\ga(\ov{e}),\bta) - W_\text{ML}(\ga(\ov{e}),\btw) \right]}{\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^a) - \pi(\tau^{tw}) \right] - \frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^b) - \pi(\tau^{a}) \right]}
 	\label{eq:e_de}
\end{equation}
...
\begin{equation}
	...
	\textstyle \frac{\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\left[ \pi(\tau^a) - \pi(\tau^{tw}) \right] - \left[ \pi(\tau^b) - \pi(\tau^{a}) \right]}{\frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^a) - \pi(\tau^{tw}) \right] -  \frac{\partial \ga}{\partial \ov{e}}\left[ \pi(\tau^b) - \pi(\tau^{a}) \right]}
\end{equation}
