...
\begin{itemize}
	...
	\item And, given this $\ov{e}$, the lobby's constraint binds
		...
		\end{itemize}
	\item In terms of Lemma 4 (old Lemma 3): when $\tau^a_{NL} < \tau^{tw}$ (recall, at $\tau^a_{NL}$ there is no lobby, so $\ov{e} = 0$), I say ``Lobby will increase profits.'' This is not necessarily true. If if the gain is negative, the constraint becomes tighter up to $\ov{e} = e_{tw}$.
		\begin{itemize}
			\item If A.2 holds at $\tau^{tw}$ (only when $\tau^a$ is such that lobby doesn't care about triggering trade war), then $\tau^a_{NL}$ is solution
			\item If A.2 fails, need to raise $\tau^a$
				\begin{itemize}
					\item Is there a condition that would guarantee that eventually raising $\tau^a$ makes the constraint hold?
				\end{itemize}
		\end{itemize}
\end{itemize}

\vskip.5in
Note (July 8, 2015): I went back to the original constraint after running some numerical examples and realizing that this construction doesn't work. It's because of what I learned in the PE$\_$shocks project: welfare is everywhere increasing in $e$, and this effect is very powerful. It means that the legislature really like the trade war---it's getting hit with higher effort in the trade war phase.	
\end{document}