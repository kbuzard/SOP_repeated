...
\begin{itemize}
\renewcommand\labelitemi{-}
	\item \textit{That $\ov{e}$ must be greater than $e_{tw}$ is a requirement for a non-trivial trade agreement to be enforceable (see Footnote 22 and the three paragraphs leading up to Result 1), because the \textit{lobby} would always prefer $e_{tw}$ to any lower effort level.}
\end{itemize}
