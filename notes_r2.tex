	...
	\item Sectioning from my paper:
		\begin{itemize}
			...
			\item[4] I think the break phase is essentially the same, but the way it is calculated is different
				\begin{itemize}
					\item Lobby first determines $\ov{e}$
						\begin{itemize}
							...
							\item When lobby has all the bargaining power, there's still a schedule $(\ov{e},\tau^b)$ that makes leg indifferent between breaking the trade agreement or abiding by it ($\tau^b$ no longer a direct fcn of $\ov{e}$)
							\begin{multline}
								W(\bta) + g(e) + \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}}\left[W(\bta) + g(e)\right] = \\
	W(\tau^b,\tau^{*a}) +g(e) + \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} \left[W(\btw) + g(e)\right]
							\end{multline}
							\[
								W(\bta) + \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} W(\bta)  = 
	W(\tau^b,\tau^{*a}) + \frac{\de_\text{ML} - \de_\text{ML}^{T+1}}{1-\de_\text{ML}} W(\btw) 
							\]

						\end{itemize}
					\item Lobby then decides whether it's worthwhile paying $\ov{e}$
				\end{itemize}
		\end{itemize}
\end{itemize}
