...
Neither the model nor any proofs have changed aside from fixing some typographical errors. I have, however, added analysis of three related models in a new Appendix B. I would characterize the remaining changes as improving the contextualization, argumentation and notation. \\

One significant change is not noted below: I have slightly clarified the discussion around Results 2 and 3 in the four paragraphs following Equation 15 on pages 28-29. I realized that it could be more transparent when re-examining the results under the alternative models. \\
...
\renewcommand\labelitemi{-}
	...
	\item \textit{I have labeled any $e$ that is not meant to describe a general functional form such as in Assumption 1 as $e_b$. This is what you will now see, for instance, in Equations (10) and (11). Note that the value of $e_b$ that is generally of most interest is $\ov{e}(\bta)$, and that in equilibrium $e_b =0$.}
\end{itemize}
...
\renewcommand\labelitemi{-}
	...
	\item \textit{I have also verified that \emph{none} of the results of the model change if lobbying effort is subtracted from the legislative/executive welfare function (except, of course, the evaluated level of welfare). I have refrained from adding the ``$-e$'' to the welfare function out of a desire to be consistent with the existing literature, as I state at the very end of Section 2.2 on page 10. The only other cost of adding it in would be the obvious additional complication to the mathematical expressions.}
		\begin{itemize}
				\item \textit{I have not supplied the analysis because (a) it is straightforward and (b) I thought it would be a bit much since the analysis of three new alternative models are already included in the revision.}
			\item \textit{It \emph{is} interesting that in the unitary model (Appendix B.1), this change would loosen the legislative constraint by making the future punishment harsher at any given $\bta$.}
		\end{itemize}
\end{itemize}
...
\renewcommand\labelitemi{-}
	...
	\item \textit{Section 3.1 page 14 also contains a discussion of the relationship between my model and Maggi and Rodriguez-Clare 2007.}
	\item \textit{I have added a references to both discussions on pages 2-3 of the introduction.}
	\item \textit{To support these discussions, I have added Appendix B.1 with analysis of the unitary model and Appendix B.3 with analysis of the strong binding case. There is additional discussion in the introduction to Appendix B.}
\end{itemize}
...
\renewcommand\labelitemi{-}
	...
	\item \textit{I have also removed 'dispute settlement' from the paper's title.}
